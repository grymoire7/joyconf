...
| `lib/joyconf/talks/talk.ex`                    | Ecto schema + changeset                                                           |
| `lib/joyconf/talks.ex`                         | Context: `create_talk/1`, `list_talks/0`, `get_talk_by_slug/1`, `generate_slug/1` |
| `lib/joyconf/rate_limiter.ex`                  | GenServer + ETS: `allow?(session_id)`                                             |
| `lib/joyconf/qr_code.ex`                       | `to_data_uri(url)` wrapping eqrcode                                               |
| `lib/joyconf/application.ex`                   | Add `RateLimiter` to supervision tree                                             |
| `lib/joyconf_web/plugs/admin_auth.ex`          | HTTP Basic Auth plug checking `ADMIN_PASSWORD` env var                            |
| `lib/joyconf_web/router.ex`                    | Routes: `/t/:slug`, `/admin`, `/admin/talks/new`                                  |
| `lib/joyconf_web/endpoint.ex`                  | Mount `UserSocket`                                                                |
| `lib/joyconf_web/channels/user_socket.ex`      | Socket + channel route                                                            |
| `lib/joyconf_web/channels/reaction_channel.ex` | Join, PubSub subscribe, push reactions to extension                               |
| `lib/joyconf_web/live/talk_live.ex`            | Attendee page: mount, react event, PubSub, push_event                             |
| `lib/joyconf_web/live/talk_live.html.heex`     | Emoji buttons, floating emoji container                                           |
| `lib/joyconf_web/live/admin_live.ex`           | Talk creation form, QR generation, talk list                                      |
| `lib/joyconf_web/live/admin_live.html.heex`    | Form, QR display, talk list                                                       |
...
| File                                                  | Tests                                               |
| ----------------------------------------------------- | --------------------------------------------------- |
| `test/joyconf/talks_test.exs`                         | Changeset validation, context CRUD, slug generation |
| `test/joyconf/rate_limiter_test.exs`                  | `allow?/1` behavior                                 |
| `test/joyconf_web/plugs/admin_auth_test.exs`          | Auth plug: correct, wrong, missing password         |
| `test/joyconf_web/live/admin_live_test.exs`           | Talk creation, QR code, list, validation errors     |
| `test/joyconf_web/live/talk_live_test.exs`            | Page load, react event, rate limit, broadcast       |
| `test/joyconf_web/channels/reaction_channel_test.exs` | Join, reject unknown slug, reaction push            |
...
mix phx.new . --app joyconf --database postgres
...
```elixir
config :joyconf, :admin_password, "devpassword"
```
...
**Files:**
- Create: `lib/joyconf/talks/talk.ex`
- Create: `lib/joyconf/talks.ex`
- Create: `test/joyconf/talks_test.exs`
...
```bash
mix test test/joyconf/talks_test.exs
```
...
```bash
git add lib/joyconf/talks.ex lib/joyconf/talks/talk.ex test/joyconf/talks_test.exs
...
**Files:**
- Create: `lib/joyconf/rate_limiter.ex`
- Modify: `lib/joyconf/application.ex`
- Create: `test/joyconf/rate_limiter_test.exs`
...
```bash
mix test test/joyconf/rate_limiter_test.exs
```
...
In `lib/joyconf/application.ex`, add to the `children` list before `JoyconfWeb.Endpoint`:
```elixir
Joyconf.RateLimiter,
```
...
```bash
git add lib/joyconf/rate_limiter.ex lib/joyconf/application.ex test/joyconf/rate_limiter_test.exs
...
**Files:**
- Create: `lib/joyconf_web/plugs/admin_auth.ex`
- Create: `test/joyconf_web/plugs/admin_auth_test.exs`
...
```bash
mix test test/joyconf_web/plugs/admin_auth_test.exs
```
Expected: compile error.
...
In `lib/joyconf_web/router.ex`, add alongside existing scopes:
```elixir
pipeline :admin do
  plug JoyconfWeb.AdminAuth, :require_admin
end
...
```bash
git add lib/joyconf_web/plugs/admin_auth.ex lib/joyconf_web/router.ex test/joyconf_web/plugs/admin_auth_test.exs
...
**Files:**
- Create: `lib/joyconf/qr_code.ex`
- Create: `lib/joyconf_web/live/admin_live.ex`
- Create: `lib/joyconf_web/live/admin_live.html.heex`
- Create: `test/joyconf_web/live/admin_live_test.exs`
...
```bash
git add lib/joyconf/qr_code.ex lib/joyconf_web/live/admin_live.ex lib/joyconf_web/live/admin_live.html.heex test/joyconf_web/live/admin_live_test.exs
...
**Files:**
- Create: `lib/joyconf_web/live/talk_live.ex`
- Create: `lib/joyconf_web/live/talk_live.html.heex`
- Create: `assets/js/hooks/emoji_buttons.js`
- Create: `assets/js/hooks/emoji_stream.js`
- Modify: `assets/js/app.js`
- Modify: `assets/css/app.css`
- Create: `test/joyconf_web/live/talk_live_test.exs`
...
```bash
git add lib/joyconf_web/live/talk_live.ex lib/joyconf_web/live/talk_live.html.heex assets/js/hooks/ assets/js/app.js assets/css/app.css test/joyconf_web/live/talk_live_test.exs
...
**Files:**
- Modify: `lib/joyconf_web/channels/user_socket.ex`
- Create: `lib/joyconf_web/channels/reaction_channel.ex`
- Verify: `lib/joyconf_web/endpoint.ex`
- Create: `test/joyconf_web/channels/reaction_channel_test.exs`
...
Edit `lib/joyconf_web/channels/user_socket.ex`:
```elixir
...
In `lib/joyconf_web/endpoint.ex`, confirm this line exists (Phoenix generates it):
```elixir
...
```bash
git add lib/joyconf_web/channels/ test/joyconf_web/channels/reaction_channel_test.exs
...
Phoenix generates `lib/joyconf/release.ex` with a `migrate/0` function. Confirm it exists:
```bash
cat lib/joyconf/release.ex
```
...
```bash
fly ssh console -C "/app/bin/joyconf eval 'Joyconf.Release.migrate()'"
```

- [ ] **Step 8: Smoke test**

- Visit `https://joyconf.fly.dev/admin/talks/new` — browser should prompt for password
